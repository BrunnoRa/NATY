...
def exchange(type_: str, payload: dict | None = None) -> dict:
...
def main() -> int:
    process = subprocess.Popen([sys.executable, str(ROOT / "core_host.py"), "--pipe", "Naty.Core.v1"], cwd=ROOT)
    try:
        ...
        stopped = exchange("shutdown")
        assert stopped["type"] == "shutdown_ack"
        process.wait(timeout=5)
        print("OK: Named Pipe v1, ping, dashboard real e shutdown limpo.")
        return 0
    finally:
